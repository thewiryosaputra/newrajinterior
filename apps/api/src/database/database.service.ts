...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  private async ensureRuntimeSchema() {
    await this.query(`
      ALTER TABLE invitation_requests
        ...
        ADD COLUMN IF NOT EXISTS surveyor_approved_at TIMESTAMPTZ,
        ADD COLUMN IF NOT EXISTS surveyor_approved_by UUID REFERENCES users(id) ON DELETE SET NULL
    `);
  ...
  private async seedSurveyorUser() {
    const password = this.config.get<string>("SURVEYOR_PASSWORD", "newrajinterior321");
    const passwordHash = await bcrypt.hash(password, 12);
    await this.query(
      `INSERT INTO users (name, email, phone, password_hash, role, email_verified_at, whatsapp_verified_at)
       VALUES ('Surveyor', 'surveyor', 'surveyor', $1, 'surveyor', now(), now())
       ON CONFLICT (email) DO UPDATE SET
        password_hash = EXCLUDED.password_hash,
        role = 'surveyor',
        email_verified_at = COALESCE(users.email_verified_at, now()),
